...
var adapter = new Adapter (objects);

var objects = adapter.objects.objects;
var states = adapter.states.states;
...
//var states = {};
//var stateIds = [];
var subscriptions = adapter.subscriptions;
...
function Sandbox () {
            ...
            callback: function (obj) {
                if (typeof callback === 'function') {
                    try {
                        ...
                        console.log(e); // adapter.log.error('Error in callback: ' + e);
                    }
                }
            },
            name: name
    ...
    this.adapterSubscribe = function (id) {
        if (typeof id !== 'string') {
    ...
    this.adapterUnsubscribe = function (id) {
        unsubscribe(id);
    };
    this.unsubscribe = function (idOrObject) {
    ...
    this.on = function (pattern, callbackOrId, value) {
        return sandbox.subscribe(pattern, callbackOrId, value);
    };
    this.schedule = function (pattern, callback) {
        if (typeof callback !== 'function') {
    ...
    this.getAstroDate = function (pattern, date, offsetMinutes) {
    ...
    this.clearSchedule = function (schedule) {
    ...
    this.setState = function (id, state, isAck, callback) {
        if (typeof isAck === 'function') {
            callback = isAck;
            isAck = undefined;
        }
    ...
    this.setStateDelayed = function (id, state, isAck, delay, clearRunning, callback) {
        // find arguments
    ...
    this.clearStateDelayed = function (id, timerId) {
        // Check type of state
        if (!objects[id] && objects[adapter.namespace + '.' + id]) {
            id = adapter.namespace + '.' + id;
        }
    ...
    this.getStateDelayed = function (id) {
        var result;
        var now = Date.now();
        if (id) {
            // Check type of state
    ...
    this.getState = function (id, callback) {
        if (typeof callback === 'function') {
            ...
            if (adapter.config.subscribe) {
                ...
                console.log('getState "' + id + '" not found (3)' + (states[id] !== undefined ? ' states[id]=' + states[id] : ''));     ///xxx
                return {val: null, notExist: true};
            }
        }
    };
    this.existsState = function (id) {
        return states.get(id) !== undefined;
    };
    this.existsObject = function (id) {
        return objects.get(id) !== undefined;
    };
    this.getIdByName = function (name, alwaysArray) {
    ...
    this.getObject = function (id, enumName) {
        if (!objects[id]) {
    ...
    this.getEnums = function (enumName) {
        var result = [];
    ...
    this.createState = function (name, initValue, forceCreation, common, native, callback) {
    ...
    this.deleteState = function (id, callback) {
        // todo: check rights
        var found = false;
        if (objects[id]) {
            found = true;
    ...
    this.sendTo = function (_adapter, cmd, msg, callback) {
    ...
    this.sendToHost = function (host, cmd, msg, callback) {
    ...
    this.setInterval = function (callback, ms, arg1, arg2, arg3, arg4) {
    ...
    this.setTimeout = function (callback, ms, arg1, arg2, arg3, arg4) {
    ...
    this.compareTime = function (startTime, endTime, operation, time) {
        var pos;
    ...
    this.writeFile = function (_adapter, fileName, data, callback) {
    ...
    this.readFile = function (_adapter, fileName, callback) {
    ...
    this.unlink = function (_adapter, fileName, callback) {
        if (typeof fileName === 'function') {
            callback = fileName;
            fileName = _adapter;
            _adapter = null;
    ...
    this.delFile = function (_adapter, fileName, callback) {
        return sandbox.unlink(_adapter, fileName, callback);
    };
    this.getHistory = function (instance, options, callback) {
    ...
    this.startScript = function (scriptName, ignoreIfStarted, callback) {
    ...
    this.stopScript = function (scriptName, callback) {
        scriptName = scriptName || name;
    ...
    this.isScriptActive = function (scriptName) {
        if (!scriptName.match(/^script\.js\./)) scriptName = 'script.js.' + scriptName;
        if (!objects[scriptName] || !objects[scriptName].common) {
